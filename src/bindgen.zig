//! `gabbro bindgen` — generate Gabbro FFI bindings from a C header using libclang.
//!
//! Parses an arbitrary C header into a clang AST, then emits idiomatic Gabbro:
...
pub fn generate(
    ...
    if (tu == null) {
        std.debug.print("gabbro bindgen: clang failed to parse '{s}'\n", .{header_path});
    ...
    std.debug.print("gabbro bindgen: wrote {s} ({d} bytes)\n", .{ out_path, text.len });
}

/// In-memory variant: parse `source` (a virtual header named `header_name`) and
/// return the generated Gabbro (caller owns the result). No system-include probing —
...
/// Walk a parsed translation unit and emit the Gabbro module text (arena-owned).
fn emitModule(arena: std.mem.Allocator, tu: c.CXTranslationUnit, lib: []const u8, label: []const u8) ![]u8 {
    ...
    try gen.out.print(arena,
        \\// Generated by `gabbro bindgen` from {s}. Do not edit by hand.
        \\
        \\#system_library("{s}");
        \\
        \\
...
const Generator = struct {
    ...
    /// A C identifier rendered as a valid Gabbro identifier: names that collide with
    /// a Gabbro keyword (`type`, `match`, `in`, `fn`, …) get a trailing `_`. Applied
    ...
    fn emitMacro(self: *Generator, cursor: c.CXCursor) !void {
        ...
        if (try self.matchCompoundLiteral(body)) |lit| {
            ...
            const gabbroname = try self.ident(name);
            try self.out.print(self.arena,
                \\__lit_{s} :: fn() -> {s} {{ return .{{ {s} }}; }}
                \\pub {s} :: #run __lit_{s}();
                \\
                \\
            , .{ gabbroname, try self.ident(lit.type_name), lit.values, gabbroname, gabbroname });
            return;
        }
    ...
    fn emitTypedef(self: *Generator, cursor: c.CXCursor) !void {
        ...
        switch (canon.kind) {
            ...
            c.CXType_Enum => {
                ...
                if (try self.markEmitted(name)) return;
                // A function-pointer typedef can't be a Gabbro type alias
    ...
    /// A C function type → Gabbro's inline function-pointer type `fn(args) -> ret`.
    /// (Gabbro has no `Name :: fn(...)` type-alias syntax, so callbacks are always
...
/// Render a C numeric/string literal token as a Gabbro literal, or null if it isn't
/// one Gabbro can take verbatim (char literals, etc.). Strips C integer/float
/// suffixes (`u`, `L`, `f`, …) but preserves hex digits.
fn cleanLiteral(text: []const u8) ?[]const u8 {
    if (text.len == 0) return null;
    if (text[0] == '"') return text; // string literal — valid Gabbro as-is
...
fn reportDiagnostics(tu: c.CXTranslationUnit, header_path: []const u8) void {
    ...
    while (i < n) : (i += 1) {
        ...
        if (cl.clang_getCString(cxs)) |s|
            std.debug.print("gabbro bindgen: {s}\n", .{std.mem.span(s)});
    }
    if (errors != 0)
        std.debug.print("gabbro bindgen: {s} parsed with {d} clang error(s); bindings may be incomplete\n", .{ header_path, errors });
}
...
///   2. `$GABBRO_LLVM/lib/clang/<v>/include` — a user-pointed LLVM install
...
fn clangResourceInclude(arena: std.mem.Allocator, io: std.Io, exe_dir: ?[]const u8) ?[]const u8 {
    // 1. Component layout: a flat include dir shipped beside gabbro.
    if (exe_dir) |ed| {
