...
test "enum: simple declaration and value access" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "enum.sk", src);
...
test "enum: match generates variant_is instructions" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "color.sk", src);
    defer fe.deinit(arena.allocator());
    const m = try k2.lowerFrontend(arena.allocator(), fe);
    try k2.ir_mod.validateModule(m);
...
test "enum: payload variant with binding" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "msg.sk", src);
    defer fe.deinit(arena.allocator());
    const m = try k2.lowerFrontend(arena.allocator(), fe);
    try k2.ir_mod.validateModule(m);
...
test "function pointers: declare, pass, and call through local" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "fnptr.sk", src);
    defer fe.deinit(arena.allocator());
    const m = try k2.lowerFrontend(arena.allocator(), fe);
    try k2.ir_mod.validateModule(m);
}
...
test "self-referencing type: linked list node" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "linked.sk", src);
    defer fe.deinit(arena.allocator());
    const m = try k2.lowerFrontend(arena.allocator(), fe);
    try k2.ir_mod.validateModule(m);
...
test "integer match: single and grouped values lower to comparisons" {
    ...
    const src =
        ...
        \\}
    ;
    var fe = try k2.compile(arena.allocator(), "int_match.sk", src);
    defer fe.deinit(arena.allocator());
    const m = try k2.lowerFrontend(arena.allocator(), fe);
    try k2.ir_mod.validateModule(m);
...
test "integer match: enum pattern is rejected" {
    ...
    const bad =
        ...
        \\}
    ;
    try std.testing.expectError(error.SemanticFailed, k2.compile(arena.allocator(), "bad_int_match.sk", bad));
}
...
test "enum match: integer pattern is rejected" {
    ...
    const bad =
        ...
        \\}
    ;
    try std.testing.expectError(error.SemanticFailed, k2.compile(arena.allocator(), "bad_enum_match.sk", bad));
}
